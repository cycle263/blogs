...
* 必会题
  ...
  - script标签中的defer和async属性有什么区别？  答案：async是下载完就执行，defer是渲染完再执行，多个脚本情况下，defer可以保证加载顺序。
...
* 高级必会题
  ...
  - commonJs和ES 6的模块有什么区别？  答案：commonjs的模块是值的拷贝，es 6的模块是值的引用；commonjs的模块是动态分析，运行时加载，输入输出，而es 6模块是静态分析，编译时加载，输入输出。
