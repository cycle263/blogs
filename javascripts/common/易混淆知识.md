...
* 5、Mouse事件中各坐标（X\Y）含义
    ...
    **元素相对视窗的距离**
    ...
    - getBoundingClientRect方法，返回相对于viewport的位置及元素的大小。

    - getClientRects方法，返回元素的数个矩形区域相对于viewport的位置及元素的大小。该方法主要用于内联元素，内联元素有多少行，该方法返回的对象有多少个成员，可用于判断行内元素是否换行，以及行内元素的每一行的位置偏移。
