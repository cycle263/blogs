...
### 执行任务（执行栈）
  ...
  * 堆
    ...
    堆（heap）用于复杂数据类型（引用类型）分配空间，例如数组对象、object对象（引用类型的值通常大小不固定，所以被存储在堆内存中）。它是运行时动态分配内存的，大小不定，也不会自动释放，存取速度较慢。指那些可能由多个值构成的对象，保存在堆内存中，包含引用类型的变量，实际上保存的不是变量本身，而是指向该对象的指针。
